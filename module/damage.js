...
export class Damage {
    ...
    static _showDetails(data) {
        ...
        if (str == 0) {
            ChatMessage.create({
                user: game.user._id,
                speaker: { actor },
                content: '<strong>' + game.i18n.localize('WYRM.Dead') + '</strong>',
            }, {});
            return;
        }

        let content = '<p><strong>' + game.i18n.localize('WYRM.Damage') + '</strong>: ' + dmg + ' (' + damage + '-' + armor + ')</p>'
        if (newHp !== hp) {
            content += '<p><strong>' + game.i18n.localize('WYRM.HitProtection') + '</strong>: <s>' + hp + '</s> => ' + newHp + '</p>'
        } else {
            content += '<p><strong>' + game.i18n.localize('WYRM.HitProtection') + '</strong>: ' + hp + '</p>'
        }
        if (newStr !== str) {
        ...
        if (newStr < str) {
            if (newStr === 0) {
                ...
                content += '<p><strong>' + game.i18n.localize('WYRM.StrSave') + '</strong></p>'
                content += '<button type="button" class="roll-str-save">' + game.i18n.localize('WYRM.RollStrSave') + '</button>'
            }
        } else if (newHp === 0 && hp !== 0) {
            content += '<p><strong>' + game.i18n.localize('WYRM.Scars') + '</strong></p>'
    ...
    static async _rollScarsTable(damage) {
        const table = await findCompendiumItem("wyrm.utils", "Scars");
    ...
    static async _rollStrSave(actor, html) {
        ...
        const label = game.i18n.format("WYRM.Save", { key: game.i18n.localize("STR") });
